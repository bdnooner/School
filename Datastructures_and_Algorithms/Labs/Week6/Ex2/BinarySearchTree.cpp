...
{
	string contents;
	string *contentsPtr = &contents;
	inorderDisplay(root, contentsPtr);
	return contents;
}
...
void BinarySearchTree::inorderDisplay(TreeNode *p, string *contents)
{

	if (p != NULL)
	{
		inorderDisplay(p->left, contents);
		*contents += std::to_string(p->info) + ", ";
		inorderDisplay(p->right, contents);
	}
